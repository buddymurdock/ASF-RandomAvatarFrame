...
[UsedImplicitly]
internal sealed class RandomAvatarFrame : IASF, IBotConnection, IGitHubPluginUpdates {
	...
	public string Name => nameof(RandomAvatarFrame);
	public string RepositoryName => "buddymurdock/ASF-RandomAvatarFrame";
	public Version Version => typeof(RandomAvatarFrame).Assembly.GetName().Version ?? throw new InvalidOperationException(nameof(Version));

	// Reads RandomAvatarFrameEnabled / RandomAvatarFrameMinDelayDays / RandomAvatarFrameMaxDelayDays from the global ASF.json config
	public Task OnASFInit(IReadOnlyDictionary<string, JsonElement>? additionalConfigProperties = null) {
		if (additionalConfigProperties != null) {
			foreach ((string configProperty, JsonElement configValue) in additionalConfigProperties) {
				switch (configProperty) {
					case $"{nameof(RandomAvatarFrame)}Enabled" when configValue.ValueKind is JsonValueKind.True or JsonValueKind.False:
						Enabled = configValue.GetBoolean();

						break;
					case $"{nameof(RandomAvatarFrame)}MinDelayDays" when (configValue.ValueKind == JsonValueKind.Number) && configValue.TryGetUInt16(out ushort minDelay) && (minDelay > 0):
						MinDelayInDays = minDelay;

						break;
					case $"{nameof(RandomAvatarFrame)}MaxDelayDays" when (configValue.ValueKind == JsonValueKind.Number) && configValue.TryGetUInt16(out ushort maxDelay) && (maxDelay > 0):
		...
		if (!Enabled) {
			ASF.ArchiLogger.LogGenericInfo($"{Name} is disabled, set {nameof(RandomAvatarFrame)}Enabled to true in ASF.json to turn it on.");

			return Task.CompletedTask;
		}

		ASF.ArchiLogger.LogGenericInfo($"{Name} is enabled, every {MinDelayInDays}-{MaxDelayInDays} days (approximate, see README) each bot randomly equips one of its own already-owned avatar frames.");
	...
	public Task OnBotLoggedOn(Bot bot) {
		...
		Utilities.InBackground(() => BotFrameLoopAsync(bot, cts.Token), true);

		return Task.CompletedTask;
	}

	private async Task BotFrameLoopAsync(Bot bot, CancellationToken cancellationToken) {
		while (!cancellationToken.IsCancellationRequested) {
			...
			try {
				await TrySetRandomFrameAsync(bot).ConfigureAwait(false);
			} catch (Exception e) {
	...
	private async Task TrySetRandomFrameAsync(Bot bot) {
		string? token = bot.AccessToken;

		if (string.IsNullOrEmpty(token)) {
			bot.ArchiLogger.LogGenericWarning($"{nameof(RandomAvatarFrame)} needs a valid access token, but none is available for this bot yet.");
		...
		List<string> candidates = [.. (owned?.AvatarFrames ?? []).Where(static item => !string.IsNullOrEmpty(item.CommunityItemID)).Select(static item => item.CommunityItemID!)];

		if (candidates.Count == 0) {
			if (!EmptyInventoryWarningLogged) {
				EmptyInventoryWarningLogged = true;

				ASF.ArchiLogger.LogGenericWarning($"{nameof(RandomAvatarFrame)} couldn't find any owned avatar frames on any bot - these come from crafting Steam trading card badges, nothing to equip until at least one bot has some.");
			}
		...
		List<string> pool = (candidates.Count > 1) && (lastItemID != null) ? [.. candidates.Where(item => item != lastItemID)] : candidates;

		string communityItemID = pool[Random.Shared.Next(pool.Count)];

		bool success = await EquipFrameAsync(bot, token, communityItemID).ConfigureAwait(false);

		if (success) {
			BotLastItemID[bot.BotName] = communityItemID;

			bot.ArchiLogger.LogGenericInfo($"Randomly equipped avatar frame {communityItemID}.");
		} else {
			bot.ArchiLogger.LogGenericWarning($"Failed to equip avatar frame {communityItemID}.");
		}
	}

	// SetAvatarFrame authenticates purely via access_token in the query string (same modern IPlayerService
	// family as the rest of Steam's post-2020 profile customization), so this deliberately skips ASF's
	// usual sessionid injection (ESession.None) - it's neither needed nor sent. Payload shape (input_json
	// with a communityitemid string field) is verified against chr233/ASFEnhance's already-working
	// ActivateProfileModifierItem/SetProfileTheme calls against this same endpoint family, not guessed.
	private static async Task<bool> EquipFrameAsync(Bot bot, string token, string communityItemID) {
		Uri request = new(SteamApiURL, $"/IPlayerService/SetAvatarFrame/v1/?access_token={token}");
	...
	private sealed record ProfileItemsOwnedResponse([property: JsonPropertyName("avatar_frames")] List<ProfileItemData>? AvatarFrames);
